...
class DimensionBuilder:
    ...
    def __init__(self):
        ...
        #2"""Create a complete and FK-safe date dimension"""

        logger.info("Building date dimension...")

        if "sale_date" not in sales_df.columns:
            raise ValueError("Sales data must contain 'sale_date' column")

        # NEVER mutate upstream data silently
        sales_dates = pd.to_datetime(sales_df["sale_date"]).dt.date
        ...
        # Generate FULL contiguous date range (NO GUESSWORK)
        ...
        sales_date_keys = {
            int(d.strftime("%Y%m%d")) for d in pd.to_datetime(sales_dates)
        }
        dim_date_keys = set(date_dim["date_key"])

        missing_keys = sales_date_keys - dim_date_keys
        if missing_keys:
            raise ValueError(

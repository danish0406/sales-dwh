...
class DataCleaner:
    ...
    def clean_product_data(self, product_df):
        ...
        clean_df["base_price"] = pd.to_numeric(
            clean_df.get("base_price", 0), errors="coerce"
        )
    ...
    def validate_data_quality(self):
        ...
        if "sales" in self.cleaned_data:
            ...
            if len(negative_amounts) > 0:
                issues.append(
                    f"Found {len(negative_amounts)} sales with negative amounts"
                )
            ...
            if len(missing_customer) > 0:
                issues.append(
                    f"Found {len(missing_customer)} sales missing customer_id"
                )
        ...
        if "customers" in self.cleaned_data:
            ...
            invalid_emails = customers_df[
                ~customers_df["email"].str.contains("@", na=False)
            ]
            if len(invalid_emails) > 0:
                issues.append(
                    f"Found {len(invalid_emails)} customers with invalid email format"
                )

...
class FactTableCreator:
    ...
    def create_fact_table(self, sales_df, dimensions, db_connection=None):
        ...
        if "dim_date" in dimensions:
            ...
            logger.info(
                f"✅ Joined with date dimension: {fact_df['date_key'].notna().sum()}/{len(fact_df)} matches"
            )
        else:
        ...
        if "dim_product" in dimensions:
            ...
            fact_df = fact_df.merge(
                product_dim[["product_id", "product_key"]], on="product_id", how="left"
            )
            logger.info(
                f"✅ Joined with product dimension: {fact_df['product_key'].notna().sum()}/{len(fact_df)} matches"
            )
        else:
            logger.warning(
                "Product dimension not found, creating product_key from product_id"
            )
            fact_df["product_key"] = fact_df["product_id"].apply(
                lambda x: (
                    int(x[1:]) if isinstance(x, str) and x.startswith("P") else None
                )
            )
        ...
        if "dim_customer" in dimensions:
            ...
            logger.info(
                f"✅ Joined with customer dimension: {fact_df['customer_key'].notna().sum()}/{len(fact_df)} matches"
            )
        else:
            logger.warning(
                "Customer dimension not found, creating customer_key from customer_id"
            )
            fact_df["customer_key"] = fact_df["customer_id"].apply(
                lambda x: (
                    int(x[4:]) if isinstance(x, str) and x.startswith("CUST") else None
                )
            )
        ...
        if db_connection:
            try:
                ...
                fact_df["salesperson_key"] = fact_df["region"].map(
                    region_to_salesperson
                )
                logger.info(
                    f"✅ Assigned salesperson keys: {fact_df['salesperson_key'].notna().sum()}/{len(fact_df)} assigned"
                )
        ...
        if (
            "dim_product" in dimensions
            and "cost_price" in dimensions["dim_product"].columns
        ):
            product_costs = dimensions["dim_product"].set_index("product_id")[
                "cost_price"
            ]
            fact_df["cost_price"] = fact_df["product_id"].map(product_costs)
            fact_df["profit"] = fact_df["total_amount"] - (
        ...
        final_fact_df = final_fact_df.dropna(
            subset=["date_key", "product_key", "customer_key", "salesperson_key"]
        )

        if initial_count != len(final_fact_df):
            logger.warning(
                f"Removed {initial_count - len(final_fact_df)} rows with missing foreign keys"
            )
        ...
        logger.info(
            f"   Average order value: ${final_fact_df['total_amount'].mean():,.2f}"
        )
...
def test_fact_table_creator():
    ...
    customer_dim = pd.DataFrame(
        {"customer_id": ["CUST001", "CUST002", "CUST003"], "customer_key": [1, 2, 3]}
    )

    salesperson_dim = pd.DataFrame(
        {"salesperson_key": [1, 2, 3, 4], "region": ["North", "South", "East", "West"]}
    )

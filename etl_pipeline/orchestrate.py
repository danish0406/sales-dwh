    ...
    def run_etl_pipeline(self):
        ...
        try:
            ...
            if raw_data:
                ...
                logger.info(
                    f"✅ Extracted {len(raw_data['sales'])} sales, {len(raw_data['customers'])} customers, {len(raw_data['products'])} products"
                )
            else:
    ...
    def _create_fact_table(self, sales_df, dimensions):
        ...
        try:
            ...
            if fact_table is not None:
                ...
                self.stats["Total Revenue"] = (
                    f"${fact_table['total_amount'].sum():,.2f}"
                )
                self.stats["Total Profit"] = f"${fact_table['profit'].sum():,.2f}"
                self.stats["Avg Order Value"] = (
                    f"${fact_table['total_amount'].mean():,.2f}"
                )
    ...
    def _load_to_database_safely(self, dimensions, fact_table):
        ...
        try:
            ...
            for table_name, df in dimension_loaders:
                ...
                success = loader.load_dimension_table(
                    df, table_name, truncate_first=False
                )
                if not success:
            ...
            success = loader.load_fact_table(
                fact_table, "fact_sales", truncate_first=False
            )
            if not success:
...
def main():
    ...
    orchestrate = ETLOrchestrate()
    success = orchestrate.run_etl_pipeline()

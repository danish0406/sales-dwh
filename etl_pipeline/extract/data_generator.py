...
class SalesDataGenerator:
    ...
    def _get_last_names(self):
        ...
        return [
            ...
            'Rogers', 'Cook', 'Morgan', 'Bell', 'Murphy', 'Bailey', 'Cooper', 'Richardson',
            'Cox', 'Howard', 'Ward', 'Brooks', 'Peterson', 'Gray', 'Ramirez', 'James', 'Watson',
            'Reed', 'Kelly', 'Sanders', 'Price', 'Bennett', 'Wood', 'Barnes', 'Ross', 'Henderson'
        ]
    
    def _get_products(self):
        """Define product catalog"""
        return [
            {'product_id': 'P001', 'product_name': 'Laptop Pro', 'category': 'Electronics', 'base_price': 1200.00},
            {'product_id': 'P002', 'product_name': 'Wireless Mouse', 'category': 'Accessories', 'base_price': 45.99},
            {'product_id': 'P003', 'product_name': 'Gaming Chair', 'category': 'Furniture', 'base_price': 299.99},
            {'product_id': 'P004', 'product_name': 'Monitor 27"', 'category': 'Electronics', 'base_price': 399.99},
            {'product_id': 'P005', 'product_name': 'Mechanical Keyboard', 'category': 'Accessories', 'base_price': 129.99},
            {'product_id': 'P006', 'product_name': 'Webcam HD', 'category': 'Electronics', 'base_price': 79.99},
            {'product_id': 'P007', 'product_name': 'Desk Lamp', 'category': 'Furniture', 'base_price': 39.99},
            {'product_id': 'P008', 'product_name': 'USB-C Hub', 'category': 'Accessories', 'base_price': 34.99},
        ]
    ...
    def _get_customers(self):
        ...
        for i in range(1, 101):
            ...
            customer = {
                ...
                'state': 'NY' if region == 'North' else 'CA' if region == 'West' else 'TX' if region == 'South' else 'MD',
                'country': 'USA',
                'region': region,
    ...
    def generate_sales_transactions(self):
        ...
        for i in range(self.num_transactions):
            ...
            region_salespersons = [sp for sp in self.salespersons if sp['region'] == customer['region']]
            salesperson = random.choice(region_salespersons) if region_salespersons else random.choice(self.salespersons)
        ...
        sample = df[['sale_date', 'customer_name', 'product_name', 'quantity', 'total_amount']].head(3)
        for _, row in sample.iterrows():
            print(f"   • {row['sale_date'].strftime('%Y-%m-%d')}: {row['customer_name']} bought {row['product_name']} x{row['quantity']} for ${row['total_amount']}")
    ...
    def generate_all_data(self):
        ...
        for i, customer in enumerate(self.customers[:5]):
            print(f"   • {customer['customer_name']} ({customer['customer_id']}) - {customer['city']}, {customer['region']}")
